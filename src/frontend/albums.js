...
import '../../node_modules/@lttr/skaut-design/css/skaut-design.css'
...
function renderChronoAlbums(data) {
  ...
  bind(chronoAlbumsElement)`${timeline(data)}`
}
  ...
  return wire()`
    <section class="section">
      <div class="timeline ">
        ...
        ${albums.map(album => {
          const folder = config.photosFolderName
          const date = album.albumDateString
          const name = album.photos[0].photoName
          const res = config.resolutions.thumbnail
          const imgSrc = `${folder}/${date}/${name}__${res}.jpg`
